...
- Модель сообщает роль уровня (`entry` / `stop` / `target`) и направление только
  ...
  Плашку текущей цены модель раньше путала со входом, поэтому промпт явно
  запрещает роль "entry" для текущей цены, а форма дополнительно отбрасывает
  такой уровень. После этой правки `claude-haiku-4-5` даёт 6/6 по всем полям и
  стоит по умолчанию: он примерно в семь раз дешевле Opus на том же скриншоте.
